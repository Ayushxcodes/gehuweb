-- Phase M3 / Chunk 11 (Lock Recovery & Production Restoration)
-- Purpose: 
-- 1) Aborts any active locked transactions in the database session.
-- 2) Safely terminates stale connections that are holding AccessExclusiveLocks on mocks tables.
-- 3) Restores the high-performance, fully-qualified student feeds with clean production filters (results_published = true).

-- =========================================================================
-- STEP 1: LOCK RECOVERY (Terminates all other stale connections holding locks)
-- =========================================================================
rollback; -- End any current uncommitted transaction in this window
...
-- =========================================================================
-- STEP 2: RESTORE CLEAN PRODUCTION FEEDS (Only show published test results)
-- =========================================================================
begin;

-- A. Clean Results Feed (Only show published tests, fully qualified to avoid ambiguity)
...
begin
  ...
  vt as (
    ...
    where mt.results_published = true -- RESTORE PRODUCTION FILTER: Only show published scores!
  )
  select
...
-- B. Clean Mock Feed (Only show posted tests, fully qualified to avoid ambiguity)
...
begin
  ...
  vt as (
    select mt.*
    from mocks.mock_tests mt
    where mt.status = 'POSTED' -- RESTORE PRODUCTION FILTER: Only show active posted tests!
